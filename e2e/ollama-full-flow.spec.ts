...
test.describe.serial('DeltaChat Ollama Full Flow', () => {
  ...
  test('Step 3: Create a chat, select model, and send a message', async () => {
    ...
    const textarea = page.locator(
      '.chat-textarea textarea:not([readonly]):not([aria-hidden="true"])',
    )
    ...
    const sendBtn = page
      .locator('.chat-input-actions .v-btn[aria-disabled="false"]')
      .filter({ has: page.locator('.mdi-send') })
