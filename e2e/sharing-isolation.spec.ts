...
test.describe.serial('Sharing & Isolation', () => {
  ...
  test('Unsharing removes access', async () => {
    // Admin unshares
    const delRes = await api.delete(`${API}/sharing/${shareId}`, {
      headers: authHeaders(admin.token),
    })
  ...
  test('User B cannot delete User A model', async () => {
    const res = await api.delete(`${API}/models/${userAModelId}`, {
      headers: authHeaders(userB.token),
    })
    expect(res.status()).toBe(403)
  })
  ...
  test('User B cannot delete User A chat', async () => {
    const res = await api.delete(`${API}/chats/${userAChatId}`, {
      headers: authHeaders(userB.token),
    })
    expect(res.status()).toBe(403)
  })
  ...
  test('Cleanup test data', async () => {
    // Delete chats
    await api
      .delete(`${API}/chats/${userAChatId}`, { headers: authHeaders(userA.token) })
      .catch(() => {})
    await api
      .delete(`${API}/chats/${userBChatId}`, { headers: authHeaders(userB.token) })
      .catch(() => {})

    // Delete admin model
    await api
      .delete(`${API}/models/${adminModelId}`, { headers: authHeaders(admin.token) })
      .catch(() => {})

    // Delete group
    await api
      .delete(`${API}/user-groups/${groupId}`, { headers: authHeaders(admin.token) })
      .catch(() => {})
  })
})
