...
describe('SharingService', () => {
  ...
  describe('filterAccessible', () => {
    ...
    test('admin sees all resources', async () => {
      const result = await service.filterAccessible(adminUser.id, adminUser.role, 'ai_model', allModels);
      expect(result).toEqual(allModels);
    });

    test('regular user sees only owned resources (not legacy, not others)', async () => {
      const result = await service.filterAccessible(regularUser.id, regularUser.role, 'ai_model', allModels);
    ...
    test('regular user sees directly shared resources', async () => {
      mockAdapter.getResourceSharesForTarget.mockResolvedValue([
        { id: 'share-1', resourceType: 'ai_model', resourceId: 'model-4', targetType: 'user', targetId: 'user-1' },
      ]);
    ...
    test('regular user sees group-shared resources', async () => {
      ...
      mockAdapter.getResourceSharesForTarget
        ...
        .mockResolvedValueOnce([   // group shares
          { id: 'share-2', resourceType: 'ai_model', resourceId: 'model-2', targetType: 'group', targetId: 'group-1' },
        ]);
    ...
    test('legacy resources without ownerId are NOT visible to regular users', async () => {
      const result = await service.filterAccessible(regularUser.id, regularUser.role, 'ai_model', [legacyModel]);
  ...
  describe('getAccessibleResourceIds', () => {
    ...
    test('includes directly shared resource IDs', async () => {
      mockAdapter.getResourceSharesForTarget.mockResolvedValue([
        { id: 's1', resourceType: 'ai_model', resourceId: 'model-4', targetType: 'user', targetId: 'user-1' },
      ]);
    ...
    test('includes group-shared resource IDs', async () => {
      ...
      mockAdapter.getResourceSharesForTarget
        .mockResolvedValueOnce([]) // user shares
        .mockResolvedValueOnce([   // group shares for g1
          { id: 's2', resourceType: 'ai_model', resourceId: 'model-2', targetType: 'group', targetId: 'g1' },
        ]);
    ...
    test('does not include unrelated resource type shares', async () => {
      mockAdapter.getResourceSharesForTarget.mockResolvedValue([
        { id: 's3', resourceType: 'agent', resourceId: 'agent-99', targetType: 'user', targetId: 'user-1' },
      ]);
  ...
  describe('shareResource', () => {
    test('creates a share', async () => {
      const expected = { id: 'new-share', resourceType: 'ai_model', resourceId: 'model-1', targetType: 'user', targetId: 'user-2', sharedById: 'admin-1' };
      ...
      expect(mockAdapter.createResourceShare).toHaveBeenCalledWith(
        expect.objectContaining({ resourceType: 'ai_model', resourceId: 'model-1', targetType: 'user', targetId: 'user-2' })
      );
      expect(result).toEqual(expected);
    });

    test('returns existing share if duplicate', async () => {
      const existing = { id: 'dup-share', resourceType: 'ai_model', resourceId: 'model-1', targetType: 'user', targetId: 'user-2' };
