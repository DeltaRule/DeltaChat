...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const filtered = chats.filter((c: any) => canAccessChat(c, req.user!.id, req.user!.role))
    res.json(filtered)
  } catch (err) {
    next(err)
  }
...
router.patch('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const chat = await req.services.chatService.updateChat(
...
router.post('/:id/messages', async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (wantsStream) {
    ...
    await req.services.chatService.streamMessage(
      ...
      content as string,
      {
        ...
        onDone(_fullContent, message) {
          ...
          req.services.webhookService.notify('message.created', chatId, { message }).catch(() => {})
        },
        onError(err) {
    ...
    try {
      const { userMessage, assistantMessage } = await req.services.chatService.sendMessage(
        chatId,
        content as string,
        opts,
      )
