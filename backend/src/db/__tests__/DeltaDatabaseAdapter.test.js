...
// Set DELTA_DB_URL before requiring the module so that config reads it
process.env.DELTA_DB_URL = 'http://localhost:8080';
process.env.DELTA_DB_ADMIN_KEY = 'testkey';
process.env.DELTA_DB_DATABASE = 'testdb';

const { DeltaDatabaseAdapter } = require('../DeltaDatabaseAdapter');
...
describe('DeltaDatabaseAdapter', () => {
  ...
  describe('constructor', () => {
    test('throws if DELTA_DB_URL is not set', () => {
      ...
      const { DeltaDatabaseAdapter: Adapter } = require('../DeltaDatabaseAdapter');
