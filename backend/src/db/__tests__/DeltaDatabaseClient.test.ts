...
describe('DeltaDatabaseClient', () => {
  ...
  describe('insert', () => {
    ...
    test('throws when a required non-auto field is missing (schema validation)', async () => {
      await expect(client.insert('chats', { id: 'c1' })).rejects.toMatchObject({ status: 400 })
    })
  })
  ...
  describe('findAll', () => {
    ...
    test('returns all entities listed in the index', async () => {
      mock
        .onGet('/entity/testdb.chats')
        .replyOnce(200, { keys: ['c1', 'c2'] })
        .onGet('/entity/testdb.chats')
        .replyOnce(200, { id: 'c1', title: 'A' })
        .onGet('/entity/testdb.chats')
        .replyOnce(200, { id: 'c2', title: 'B' })
      const result = await client.findAll('chats')
  ...
  describe('delete', () => {
    test('uses DELETE endpoint and removes entity from the master index', async () => {
      ...
      mock
        .onGet('/entity/testdb.chats')
        .replyOnce(200, existing)
        .onDelete('/entity/testdb.chats')
        .reply(200)
        .onGet('/entity/testdb.chats')
        .replyOnce(200, { keys: ['c1'] })
        .onPut('/entity/testdb.chats')
        .reply(200)
  ...
  describe('query', () => {
    ...
    test('uses secondary index when available', async () => {
      ...
      mock
        .onGet('/entity/testdb.messages')
        .replyOnce(200, { keys: ['m1'] })
        .onGet('/entity/testdb.messages')
        .replyOnce(200, msg)
