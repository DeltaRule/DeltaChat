...
describe('DeltaDatabaseAdapter', () => {
  ...
  describe('constructor', () => {
    test('throws if DELTA_DB_URL is not set', () => {
      ...
      const { DeltaDatabaseAdapter: Adapter } = require('../DeltaDatabaseAdapter') as {
        DeltaDatabaseAdapter: new () => DeltaDatabaseAdapter
      }
  ...
  describe('collection methods', () => {
    ...
    test('deleteMessagesByChatId deletes each message and prunes secondary index', async () => {
      ...
      expect(mockBackend.delete).toHaveBeenCalledWith('messages', 'm1', [
        { field: 'chatId', value: 'c1' },
      ])
    })
    ...
    test('updateSettings inserts when no existing record', async () => {
      ...
      expect(mockBackend.insert).toHaveBeenCalledWith('settings', { id: 'global', theme: 'dark' })
    })
