    ...
    await chatService.streamMessage(
      ...
      content,
      {
        ...
        onDone(_fullContent, message, sources) {
          ...
          webhookService.notify('message.created', chatId, { message }).catch(() => {})
        },
        onError(err) {
    ...
    try {
      ...
      if (firstAdmin) {
        const collections: Array<{
          ...
          {
            name: 'chats',
            list: () => adapter.listChats(),
            update: (id, d) => adapter.updateChat(id, d),
          },
          {
            name: 'ai_models',
            list: () => adapter.listAiModels(),
            update: (id, d) => adapter.updateAiModel(id, d),
          },
          {
            name: 'agents',
            list: () => adapter.listAgents(),
            update: (id, d) => adapter.updateAgent(id, d),
          },
          {
            name: 'tools',
            list: () => adapter.listTools(),
            update: (id, d) => adapter.updateTool(id, d),
          },
          {
            name: 'knowledge_stores',
            list: () => adapter.listKnowledgeStores(),
            update: (id, d) => adapter.updateKnowledgeStore(id, d),
          },
        ]
        for (const col of collections) {
          ...
          if (migrated > 0) {
            logger.info(
              `[Migration] Assigned ${migrated} orphaned ${col.name} to admin ${firstAdmin['email']}`,
            )
          }
        }
      }
