...
type PresetName = 'emerald' | 'purple' | 'blue' | 'green' | 'rose' | 'orange' | 'cyan'

const presets: Record<PresetName, ThemeColors> = {
...
export const useThemeStore = defineStore('theme', () => {
  ...
  const colors = ref<ThemeColors>(savedColors ? JSON.parse(savedColors) : { ...defaultColors })

  const currentPreset = computed<PresetName | 'custom'>(() => {
    const hex = colors.value.primaryHex
    for (const [name, preset] of Object.entries(presets)) {
      if (preset.primaryHex === hex) return name as PresetName
    }
    return 'custom'
  })
