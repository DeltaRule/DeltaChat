  ...
  <div class="min-h-screen flex items-center justify-center bg-background p-4">
    <div class="w-full max-w-md">
      <div class="text-center mb-8">
        <div class="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-lg mb-4">
          <Triangle class="h-6 w-6" />
        </div>
        <h1 class="text-2xl font-bold text-foreground">DeltaChat</h1>
        <p class="text-sm text-muted-foreground mt-1">{{ isRegister ? 'Create your account' : 'Sign in to continue' }}</p>
      </div>

      <div class="rounded-xl border border-border bg-card p-6 shadow-lg space-y-5">
        <!-- Error message -->
        <div v-if="error" class="rounded-lg bg-destructive/10 text-destructive text-sm px-4 py-3 flex items-center gap-2">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div v-if="isRegister" class="space-y-2">
            ...
            <Input id="name" v-model="form.name" placeholder="Your name" required autocomplete="name" />
          </div>
          ...
          <div class="space-y-2">
            ...
            <Input id="password" v-model="form.password" type="password" placeholder="••••••••" required autocomplete="current-password" :minlength="isRegister ? 8 : undefined" />
        ...
        <!-- Google Sign-In -->
        <Button v-if="googleEnabled" variant="outline" class="w-full" @click="handleGoogleLogin" :disabled="loading">
        ...
        <p class="text-center text-sm text-muted-foreground">
          ...
          <button type="button" class="ml-1 text-primary hover:underline font-medium" @click="toggleMode">
