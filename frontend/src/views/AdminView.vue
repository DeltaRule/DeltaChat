      ...
      <Tabs v-model="activeTab" class="space-y-4">
        ...
        <TabsContent value="users" class="space-y-4">
          ...
          <div v-else class="rounded-lg border border-border">
            <div class="grid grid-cols-[1fr_1fr_auto_auto_auto] gap-4 px-4 py-2 border-b border-border text-xs font-medium text-muted-foreground uppercase tracking-wide">
              <span>Name</span>
              <span>Email</span>
              <span>Role</span>
              <span>Status</span>
            ...
            <div v-if="usersStore.users.length === 0" class="px-4 py-8 text-center text-sm text-muted-foreground">
        ...
        <TabsContent value="groups" class="space-y-4">
          ...
          <div v-else-if="groupsStore.groups.length === 0" class="text-center py-12 text-sm text-muted-foreground">
            ...
            >
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p v-if="group.description" class="text-xs text-muted-foreground mt-0.5">{{ group.description }}</p>
                </div>
                <div class="flex items-center gap-1">
                  ...
                  <Button variant="ghost" size="icon-sm" class="text-destructive hover:text-destructive" @click="confirmDeleteGroup(group)">
                    <Trash2 class="h-4 w-4" />
                  </Button>
                </div>
              </div>
              ...
              <div class="space-y-2">
                ...
                <div v-if="(group.memberIds || []).length === 0" class="text-xs text-muted-foreground pl-2">No members</div>
                <div v-else class="flex flex-wrap gap-1">
                  <Badge v-for="memberId in group.memberIds" :key="memberId" variant="outline" class="gap-1">
                    {{ getUserName(memberId) }}
                    <button class="ml-1 hover:text-destructive" @click="removeMember(group.id, memberId)">×</button>
                  </Badge>
                </div>
              </div>
            </div>
    ...
    <Dialog :open="showCreateGroup" @update:open="showCreateGroup = $event">
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>{{ editingGroup ? 'Update group details' : 'Create a new user group' }}</DialogDescription>
        </DialogHeader>
        <form @submit.prevent="handleSaveGroup" class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            <Input id="group-desc" v-model="groupForm.description" placeholder="Optional description" />
          </div>
          <DialogFooter>
            <Button type="button" variant="outline" @click="showCreateGroup = false">Cancel</Button>
            <Button type="submit" :disabled="!groupForm.name.trim()">{{ editingGroup ? 'Save' : 'Create' }}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
    ...
    <Dialog :open="showAddMember" @update:open="showAddMember = $event">
      <DialogContent>
        ...
        <div class="space-y-2 max-h-64 overflow-auto">
          ...
          <div v-if="availableMembers.length === 0" class="text-sm text-muted-foreground text-center py-4">
    ...
    <Dialog :open="!!confirmDelete" @update:open="!$event && (confirmDelete = null)">
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete {{ confirmDelete?.type === 'user' ? 'user' : 'group' }}
...
import { Loader2, ShieldCheck, ShieldMinus, UserCheck, UserX, Trash2, Plus, Pencil } from 'lucide-vue-next'
