<template>
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent class="max-w-md">
      ...
      <!-- Current shares -->
      <div class="space-y-3">
        <div class="text-xs font-medium text-muted-foreground uppercase tracking-wide">Shared with</div>
        <div v-if="sharingStore.loading" class="flex justify-center py-4">
          <Loader2 class="h-4 w-4 animate-spin text-muted-foreground" />
        </div>
        <div v-else-if="sharingStore.shares.length === 0" class="text-sm text-muted-foreground text-center py-4">
        ...
        <div v-else class="space-y-1 max-h-40 overflow-auto">
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <Badge variant="outline" class="text-xs">{{ share.targetType }}</Badge>
            </div>
            <Button variant="ghost" size="icon-sm" class="text-destructive hover:text-destructive" @click="handleUnshare(share.id)">
              <X class="h-3 w-3" />
            </Button>
          </div>
        </div>
      ...
      <!-- Add share -->
      <div class="space-y-3">
        <div class="text-xs font-medium text-muted-foreground uppercase tracking-wide">Add access</div>
        <Tabs v-model="shareTab" class="space-y-3">
          ...
          <TabsContent value="user">
            <div class="space-y-2 max-h-40 overflow-auto">
              ...
              <div v-if="unsharableUsers.length === 0" class="text-sm text-muted-foreground text-center py-3">
            ...
            <div class="space-y-2 max-h-40 overflow-auto">
              ...
              >
                <div>
                  ...
                  <div class="text-xs text-muted-foreground">{{ (group.memberIds || []).length }} members</div>
                </div>
                <Plus class="h-4 w-4 text-muted-foreground" />
              </div>
              <div v-if="unsharableGroups.length === 0" class="text-sm text-muted-foreground text-center py-3">
                {{ groupsStore.groups.length === 0 ? 'No groups created yet' : 'Shared with all groups' }}
              </div>
            </div>
          </TabsContent>
        </Tabs>
...
async function handleShare(targetType, targetId) {
