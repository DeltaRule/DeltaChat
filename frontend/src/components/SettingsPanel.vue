<template>
  <div class="flex h-full overflow-hidden">
    ...
    <aside
      ref="settingsAside"
      :style="{ width: settingsRail ? '3.5rem' : settingsWidth + 'px' }"
      :class="['relative flex flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground shrink-0', isResizing ? '' : 'transition-[width] duration-300']"
    >
      <!-- Header -->
      <div v-if="settingsRail" class="flex flex-col items-center py-3 gap-1">
        <Button variant="ghost" size="icon-xs" @click="settingsRail = false">
          <ChevronRight class="h-4 w-4" />
        </Button>
      </div>
      <div v-else class="flex items-center px-3 h-12">
        <span class="text-sm font-bold flex-1">Settings</span>
        <Button variant="ghost" size="icon-xs" @click="settingsRail = true">
      ...
      <!-- Nav items -->
      <nav class="mt-1 px-1 space-y-0.5">
        <Tooltip v-for="tab in tabs" :key="tab.value" :delay-duration="settingsRail ? 200 : 99999">
          <TooltipTrigger as-child>
            <button
              :class="[
                'flex items-center gap-2 w-full rounded-md text-sm transition-colors',
                settingsRail ? 'justify-center h-9 w-full' : 'px-3 py-2',
                activeTab === tab.value ? 'bg-primary/10 text-primary border-l-2 border-primary' : 'hover:bg-sidebar-accent text-sidebar-foreground/60 hover:text-sidebar-foreground'
              ]"
              @click="activeTab = tab.value"
            >
              <component :is="tab.icon" class="h-4 w-4 shrink-0" />
              <span v-if="!settingsRail">{{ tab.label }}</span>
            </button>
          </TooltipTrigger>
      ...
      <div
        v-if="!settingsRail"
        class="absolute top-0 right-0 w-1 h-full cursor-col-resize hover:bg-primary/30 active:bg-primary/50 transition-colors z-10"
      ...
      <div class="p-6 max-w-4xl">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Card v-for="provider in providers" :key="provider.key">
              <CardHeader class="!flex !flex-row !items-center gap-3 pb-3">
                  <div :class="['flex h-9 w-9 items-center justify-center rounded-lg shrink-0', provider.bgClass]">
                  ...
                  <Switch class="shrink-0" :model-value="providerEnabled[provider.key]" @update:model-value="providerEnabled[provider.key] = $event" />
              </CardHeader>
              <CardContent v-if="providerEnabled[provider.key]" class="space-y-3">
                ...
                <div v-if="provider.hasUrl">
                  ...
                  <Input v-model="providerUrls[provider.key]" :placeholder="provider.urlLabel || 'Base URL'" />
                </div>

              </CardContent>
            </Card>
          </div>
          <Button class="mt-6 shadow-lg shadow-primary/20" :disabled="saving" @click="saveProviderSettings">
        ...
        <template v-else-if="activeTab === 'models'">
          ...
          <div class="flex items-center gap-1 mb-5 pb-3 border-b-2 border-primary/15">
            <button
              :class="['px-3 py-1.5 rounded-md text-sm font-semibold transition-colors', modelSubTab === 'general' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-accent']"
              @click="modelSubTab = 'general'"
            >Models</button>
            <button
              :class="['px-3 py-1.5 rounded-md text-sm font-semibold transition-colors', modelSubTab === 'embedding' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-accent']"
              @click="modelSubTab = 'embedding'"
            >Embedding</button>
            <div class="flex-1" />
            <Button size="sm" @click="openModelDialog(null, modelSubTab === 'embedding' ? 'embedding' : 'model')">
              <Plus class="h-4 w-4 mr-2" />{{ modelSubTab === 'embedding' ? 'New Embedding' : 'New Model' }}
            </Button>
          </div>

          <!-- Default info hint -->
          <p class="text-xs text-muted-foreground mb-3">Click the checkbox next to a model to set it as default.</p>

          <!-- General models list -->
          <template v-if="modelSubTab === 'general'">
            <p class="text-sm text-muted-foreground mb-4">
              Models are named configurations that users chat with. Includes standard models, webhooks, and agents.
            </p>
            ...
            <div v-if="!generalModels.length" class="flex flex-col items-center py-14 text-center">
              ...
              <p class="text-sm text-muted-foreground mb-6 max-w-[340px]">Create a named model configuration.</p>
              <Button @click="openModelDialog(null, 'model')"><Plus class="h-4 w-4 mr-2" />New Model</Button>
            </div>
              ...
              <div class="divide-y divide-border">
                ...
                >
                  <Tooltip :delay-duration="200">
                    <TooltipTrigger as-child>
                      <Checkbox :checked="m.id === defaultModelId" @update:checked="toggleDefaultModel(m.id)" />
                    </TooltipTrigger>
                    <TooltipContent>{{ m.id === defaultModelId ? 'Default model' : 'Set as default' }}</TooltipContent>
                  </Tooltip>
                  <component :is="m.type === 'agent' ? Bot : m.type === 'webhook' ? Webhook : Brain" class="h-4 w-4 text-muted-foreground shrink-0" />
                  <div class="flex-1 min-w-0">
          ...
          <template v-else>
            ...
            <div v-if="!embeddingModels.length" class="flex flex-col items-center py-14 text-center">
              ...
              <p class="text-sm text-muted-foreground mb-6 max-w-[340px]">Create an embedding model configuration.</p>
              <Button @click="openModelDialog(null, 'embedding')"><Plus class="h-4 w-4 mr-2" />New Embedding</Button>
            </div>
        ...
        <template v-else-if="activeTab === 'vectorstores'">
          ...
          <p class="text-sm text-muted-foreground mb-4">Configure available vector store backends and set the default for new knowledge stores.</p>
        ...
        <template v-else-if="activeTab === 'docprocessor'">
          ...
          <p class="text-sm text-muted-foreground mb-4">Configure available document processors and set the default for new knowledge stores.</p>
        ...
        <template v-else-if="activeTab === 'knowledge'">
          <div class="flex items-center justify-between mb-5 pb-3 border-b-2 border-primary/15">
            ...
            <Button size="sm" @click="showCreateKs = true"><Plus class="h-4 w-4 mr-2" />New Store</Button>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4">
            <div>
              <div v-if="!knowledgeStore.knowledgeStores.length" class="flex flex-col items-center py-10 border border-dashed border-border rounded-lg text-center">
              ...
              <Card v-else>
                <div class="divide-y divide-border">
                  <div
                    ...
                    :key="ks.id"
                    :class="['flex items-center gap-2 px-3 py-2 cursor-pointer transition-colors', selectedKs?.id === ks.id ? 'bg-primary/10 text-primary' : 'hover:bg-accent']"
                    @click="selectKs(ks)"
                  >
            ...
            <div>
                ...
                <CardContent class="space-y-4">
                  ...
                  <div v-if="ksDocs.length" class="divide-y divide-border rounded-md border">
                    <div v-for="doc in ksDocs" :key="doc.id" class="flex items-center gap-2 px-3 py-2">
                      ...
                      <Badge :variant="statusVariant(doc.status)">{{ doc.status || 'ready' }}</Badge>
                      <Button variant="ghost" size="icon-xs" class="text-destructive" @click="knowledgeStore.deleteDocument(selectedKs.id, doc.id)"><Trash2 class="h-3 w-3" /></Button>
                    </div>
                  </div>
              ...
              <div v-else class="flex flex-col items-center justify-center py-16 border border-dashed border-border rounded-lg min-h-[240px]">
        ...
        <template v-else-if="activeTab === 'agents'">
          ...
          <p class="text-sm text-muted-foreground mb-4">Agents combine a system prompt, knowledge stores, and tools.</p>
          ...
          <div v-if="!agentsStore.agents.length" class="flex flex-col items-center py-14 text-center">
            ...
            <p class="text-sm text-muted-foreground mb-6 max-w-[340px]">Agents combine a system prompt, tools, and knowledge stores.</p>
            <Button @click="openAgentDialog()"><Plus class="h-4 w-4 mr-2" />New Agent</Button>
          </div>

          <Card v-else>
            <div class="divide-y divide-border">
              <div v-for="agent in agentsStore.agents" :key="agent.id" class="flex items-center gap-3 px-4 py-3 hover:bg-accent/50 transition-colors">
        ...
        <template v-else-if="activeTab === 'tools'">
          ...
          <p class="text-sm text-muted-foreground mb-4">Connect tools via MCP, or define custom Python / TypeScript functions.</p>
          ...
          <div v-if="!toolsStore.tools.length" class="flex flex-col items-center py-14 text-center">
            ...
            <p class="text-sm text-muted-foreground mb-6 max-w-[340px]">Add MCP tool servers or define custom functions.</p>
            <Button @click="openToolDialog()"><Plus class="h-4 w-4 mr-2" />New Tool</Button>
          </div>

          <Card v-else>
            <div class="divide-y divide-border">
              <div v-for="tool in toolsStore.tools" :key="tool.id" class="flex items-center gap-3 px-4 py-3 hover:bg-accent/50 transition-colors">
                <component :is="toolIconMap[tool.type] || Wrench" class="h-4 w-4 text-muted-foreground shrink-0" />
                <div class="flex-1 min-w-0">
        ...
        <template v-else-if="activeTab === 'appearance'">
          ...
          <Card>
            <CardHeader>
              ...
              <p class="text-xs text-muted-foreground">Choose your accent color. This is saved in cookies.</p>
            </CardHeader>
            <CardContent class="space-y-4">
              <div class="flex flex-wrap gap-3">
                <button
                  ...
                  :class="[
                    ...
                    themeStore.currentPreset === name ? 'border-foreground ring-2 ring-offset-2 ring-offset-background ring-foreground scale-110' : 'border-transparent'
                  ]"
              ...
              <div>
                ...
                <div class="flex items-center gap-3">
                  <input
                    ...
                    @input="themeStore.setCustomColor($event.target.value)"
                  />
                  <Input
                    :model-value="themeStore.colors.primaryHex"
                    class="w-28 font-mono text-sm"
                    @update:model-value="v => { if (/^#[0-9a-fA-F]{6}$/.test(v)) themeStore.setCustomColor(v) }"
                  />
                </div>
              </div>
              <Separator />
              <div class="flex items-center justify-between">
                ...
                <Switch :model-value="themeStore.isDark" @update:model-value="themeStore.isDark = $event" />
              </div>
            </CardContent>
          </Card>
        </template>
    ...
    <Dialog :open="showModelDialog" @update:open="showModelDialog = $event">
      <DialogContent>
        ...
        <div class="space-y-3">
          <div><Label class="mb-1.5 block text-xs">Display Name</Label><Input v-model="modelForm.name" placeholder="Display Name" /></div>
          <div>
          ...
          <template v-if="modelForm.type === 'model' || modelForm.type === 'embedding'">
            <div>
              ...
              <Select v-model="modelForm.provider">
                ...
                <SelectContent>
                  <SelectItem v-for="p in (modelForm.type === 'embedding' ? embeddingProviderKeys : providerKeys_list)" :key="p" :value="p">{{ p }}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <template v-if="modelForm.type === 'model'">
              ...
              <div>
                ...
                <div class="max-h-32 overflow-y-auto space-y-1.5 rounded-md border p-2">
                  <div v-if="!knowledgeStore.knowledgeStores.length" class="text-xs text-muted-foreground py-1">No knowledge stores available</div>
                  <label v-for="ks in knowledgeStore.knowledgeStores" :key="ks.id" class="flex items-center gap-2 cursor-pointer rounded px-1 py-0.5 hover:bg-accent">
                    <Checkbox :checked="modelForm.knowledgeStoreIds.includes(ks.id)" @update:checked="toggleModelKs(ks.id)" />
          ...
          <template v-else-if="modelForm.type === 'webhook'">
            <div>
              ...
              <Select v-model="modelForm.webhookId">
                ...
                <SelectContent>
                  <SelectItem v-for="w in webhookItems" :key="w.value" :value="w.value">{{ w.title }}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </template>
          <template v-else-if="modelForm.type === 'agent'">
            <div>
              ...
              <Select v-model="modelForm.agentId">
                ...
                <SelectContent>
                  <SelectItem v-for="a in agentItems" :key="a.value" :value="a.value">{{ a.title }}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </template>
          <div class="flex items-center justify-between">
            <Label>Enabled</Label>
            <Switch :model-value="modelForm.enabled" @update:model-value="modelForm.enabled = $event" />
          </div>
        </div>
        <DialogFooter>
    ...
    <Dialog :open="showAgentDialog" @update:open="showAgentDialog = $event">
      <DialogContent>
        ...
        <div class="space-y-3">
          <div><Label class="mb-1.5 block text-xs">Name</Label><Input v-model="agentForm.name" placeholder="Agent name" /></div>
          <div><Label class="mb-1.5 block text-xs">Description</Label><Textarea v-model="agentForm.description" placeholder="Optional description" rows="2" /></div>
          <div>
            <Label class="mb-1.5 block text-xs">Provider</Label>
            <Select v-model="agentForm.provider">
              <SelectTrigger><SelectValue placeholder="Select provider…" /></SelectTrigger>
              <SelectContent>
                <SelectItem v-for="p in providerKeys_list" :key="p" :value="p">{{ p }}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div><Label class="mb-1.5 block text-xs">Model Name</Label><Input v-model="agentForm.providerModel" placeholder="Model name" /></div>
          <div><Label class="mb-1.5 block text-xs">System Prompt</Label><Textarea v-model="agentForm.systemPrompt" placeholder="System prompt…" rows="4" /></div>
        </div>
        <DialogFooter>
    ...
    <Dialog :open="showToolDialog" @update:open="showToolDialog = $event">
      <DialogContent>
        ...
        <div class="space-y-3">
          <div><Label class="mb-1.5 block text-xs">Name</Label><Input v-model="toolForm.name" placeholder="Tool name" /></div>
          <div><Label class="mb-1.5 block text-xs">Description</Label><Textarea v-model="toolForm.description" placeholder="Description" rows="2" /></div>
          <div>
            <Label class="mb-1.5 block text-xs">Type</Label>
          ...
          <template v-if="toolForm.type === 'mcp'">
            <div><Label class="mb-1.5 block text-xs">MCP Server URL</Label><Input v-model="toolForm.config.serverUrl" placeholder="https://..." /></div>
            <div><Label class="mb-1.5 block text-xs">Tool Name</Label><Input v-model="toolForm.config.toolName" placeholder="Tool name" /></div>
          </template>
          <template v-else>
            <div><Label class="mb-1.5 block text-xs">Function Code</Label><Textarea v-model="toolForm.config.code" placeholder="Code…" rows="6" class="font-mono text-xs" /></div>
          </template>
          <div class="flex items-center justify-between">
            <Label>Enabled</Label>
            <Switch :model-value="toolForm.enabled" @update:model-value="toolForm.enabled = $event" />
          </div>
        </div>
        <DialogFooter>
    ...
    <Dialog :open="showCreateKs" @update:open="showCreateKs = $event">
      <DialogContent class="max-w-sm">
        ...
        <div class="space-y-3">
          <div><Label class="mb-1.5 block text-xs">Name</Label><Input v-model="newKsName" placeholder="Store name" /></div>
          <div><Label class="mb-1.5 block text-xs">Description (optional)</Label><Textarea v-model="newKsDesc" placeholder="Description…" rows="2" /></div>
          <Separator />
          <p class="text-xs text-muted-foreground">Pipeline configuration (pre-filled from defaults)</p>
          <div>
            ...
            <Select v-model="newKsEmbeddingModelId">
              ...
              <SelectContent>
                ...
                <SelectItem v-for="m in embeddingModels" :key="m.id" :value="m.id">{{ m.name }}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
...
const settingsWidth = ref(208) // 13rem = 208px (w-52)
const settingsAside = ref(null)
...
function onDragStart(e) {
  ...
  const startW = settingsWidth.value
  isResizing.value = true
  function onMove(ev) {
    const delta = ev.clientX - startX
    settingsWidth.value = Math.max(140, Math.min(400, startW + delta))
  }
  function onUp() {
    isResizing.value = false
    document.removeEventListener('mousemove', onMove)
    document.removeEventListener('mouseup', onUp)
    if (settingsWidth.value < 100) settingsRail.value = true
  }
...
const toolIconMap = { mcp: Link, python: Code, typescript: Code }

// ── Provider settings ──────────────────────────────────
const providers = [
  { key: 'openai', name: 'OpenAI', keyLabel: 'API Key', icon: Brain, iconClass: 'text-emerald-500', bgClass: 'bg-emerald-500/10', description: 'GPT-4o, GPT-4, GPT-3.5 and more' },
  { key: 'anthropic', name: 'Anthropic', keyLabel: 'API Key', icon: Bot, iconClass: 'text-amber-500', bgClass: 'bg-amber-500/10', description: 'Claude 3.5 Sonnet, Claude 3 Opus' },
  { key: 'ollama', name: 'Ollama', keyLabel: 'API Key (optional)', hasUrl: true, urlLabel: 'Base URL', icon: Zap, iconClass: 'text-sky-500', bgClass: 'bg-sky-500/10', description: 'Run models locally' },
  { key: 'groq', name: 'Groq', keyLabel: 'API Key', icon: Zap, iconClass: 'text-yellow-500', bgClass: 'bg-yellow-500/10', description: 'Ultra-fast inference' },
  { key: 'gemini', name: 'Google Gemini', keyLabel: 'API Key', icon: Brain, iconClass: 'text-blue-500', bgClass: 'bg-blue-500/10', description: 'Gemini 1.5 Pro, Flash' },
  { key: 'azure', name: 'Azure OpenAI', keyLabel: 'API Key', hasUrl: true, urlLabel: 'Azure Endpoint', icon: Brain, iconClass: 'text-cyan-500', bgClass: 'bg-cyan-500/10', description: 'OpenAI on Azure' },
]
...
providers.forEach(p => {
  providerEnabled[p.key] = false; providerKeys[p.key] = ''; providerUrls[p.key] = ''; showKey[p.key] = false
})

// ── Model sub-tab & defaults ─────────────────────────
const modelSubTab = ref('general')
const defaultModelId = ref(null)
const defaultEmbeddingModelId = ref(null)
...
const providerKeys_list = computed(() => providers.filter(p => providerEnabled[p.key]).map(p => p.key))

// ── Vector Store providers ───────────────────────────
const vectorStoreProviders = [
  { key: 'local', name: 'Local (built-in)', icon: HardDrive, iconClass: 'text-green-500', bgClass: 'bg-green-500/10', description: 'In-memory vector storage, no external service needed' },
  { key: 'chroma', name: 'Chroma', icon: Database, iconClass: 'text-purple-500', bgClass: 'bg-purple-500/10', description: 'External Chroma vector database', hasUrl: true, urlLabel: 'Chroma URL', urlPlaceholder: 'http://localhost:8000' },
]
const vectorStoreEnabled = reactive({ local: true, chroma: false })
const vectorStoreUrls = reactive({ chroma: '' })
const defaultVectorStoreType = ref('local')
const enabledVectorStores = computed(() => vectorStoreProviders.filter(vs => vectorStoreEnabled[vs.key]))

// ── Document Processor providers ─────────────────────
const docProcessorProviders = [
  { key: 'langchain', name: 'LangChain (local)', icon: Code, iconClass: 'text-green-500', bgClass: 'bg-green-500/10', description: 'Local text extraction, no external service' },
  { key: 'tika', name: 'Apache Tika', icon: FileSearch, iconClass: 'text-orange-500', bgClass: 'bg-orange-500/10', description: 'External text extraction service', hasUrl: true, urlLabel: 'Tika URL', urlPlaceholder: 'http://localhost:9998' },
  { key: 'docling', name: 'Docling', icon: FileText, iconClass: 'text-blue-500', bgClass: 'bg-blue-500/10', description: 'External document processing service', hasUrl: true, urlLabel: 'Docling URL', urlPlaceholder: 'http://localhost:5001' },
]
const docProcessorEnabled = reactive({ langchain: true, tika: false, docling: false })
const docProcessorUrls = reactive({ tika: '', docling: '' })
...
watch(() => settingsStore.settings, (s) => {
  ...
  if (s.defaultEmbeddingModelId !== undefined) defaultEmbeddingModelId.value = s.defaultEmbeddingModelId
  ...
  else if (s.defaultVectorStoreConfig?.type) defaultVectorStoreType.value = s.defaultVectorStoreConfig.type
  ...
  if (s.documentProcessors) {
    docProcessorProviders.forEach(dp => {
      if (s.documentProcessors[dp.key] !== undefined) {
        ...
        if (s.documentProcessors[dp.key].url) docProcessorUrls[dp.key] = s.documentProcessors[dp.key].url
      }
    })
    ...
    if (s.defaultDocumentProcessorConfig.url) docProcessorUrls[t] = s.defaultDocumentProcessorConfig.url
  }
  if (s.defaultDocumentProcessorType) defaultDocProcessorType.value = s.defaultDocumentProcessorType
  else if (s.defaultDocumentProcessorConfig?.type) defaultDocProcessorType.value = s.defaultDocumentProcessorConfig.type
...
async function saveProviderSettings() {
  ...
  const providerData = {}
  providers.forEach(p => {
    providerData[p.key] = { enabled: providerEnabled[p.key], apiKey: providerKeys[p.key], baseUrl: providerUrls[p.key] }
  })
  try {
...
async function saveDefaults() {
  saving.value = true
  try {
    await settingsStore.saveSettings({ defaultModelId: defaultModelId.value, defaultEmbeddingModelId: defaultEmbeddingModelId.value })
    notify.success('Default saved!')
  } catch { notify.error('Failed to save default.') }
  finally { saving.value = false }
}

async function toggleDefaultModel(id) {
...
async function saveVectorStoreSettings() {
  ...
  try {
    await settingsStore.saveSettings({
      ...
      defaultVectorStoreConfig: { type: defaultVectorStoreType.value, url: vectorStoreUrls[defaultVectorStoreType.value] || '' },
    })
    notify.success('Vector store settings saved!')
  } catch { notify.error('Failed to save vector store settings.') }
  finally { saving.value = false }
}
...
async function saveDocProcessorSettings() {
  ...
  try {
    await settingsStore.saveSettings({
      ...
      // Keep backward compat fields
      defaultDocumentProcessorConfig: { type: defaultDocProcessorType.value, url: docProcessorUrls[defaultDocProcessorType.value] || '' },
    })
    notify.success('Document processor settings saved!')
  } catch { notify.error('Failed to save document processor settings.') }
  finally { saving.value = false }
}
...
const knowledgeStoreItems = computed(() => knowledgeStore.knowledgeStores.map(k => ({ title: k.name, value: k.id })))
const toolItems = computed(() => toolsStore.tools.map(t => ({ title: t.name, value: t.id })))
const webhookItems = computed(() => settingsStore.webhooks.map(w => ({ title: w.name, value: w.id })))
const agentItems = computed(() => agentsStore.agents.map(a => ({ title: a.name, value: a.id })))
...
async function saveModel() {
  ...
  try {
    if (editingModel.value?.id) { await modelsStore.updateModel(editingModel.value.id, payload); notify.success('Model updated!') }
    else { await modelsStore.createModel(payload); notify.success('Model created!') }
    showModelDialog.value = false
  } catch { notify.error('Failed to save model.') }
}

function confirmDeleteModel(model) { deletingModel.value = model; showDeleteModelDialog.value = true }
async function executeDeleteModel() {
  if (deletingModel.value) { await modelsStore.deleteModel(deletingModel.value.id); notify.success('Model deleted.') }
  showDeleteModelDialog.value = false; deletingModel.value = null
}

// ── Agent dialog ───────────────────────────────────
const showAgentDialog = ref(false)
const editingAgent = ref(null)
const agentForm = reactive({ name: '', description: '', provider: null, providerModel: '', systemPrompt: '', knowledgeStoreIds: [], toolIds: [] })

function openAgentDialog(agent = null) {
  editingAgent.value = agent
  if (agent) Object.assign(agentForm, { name: agent.name, description: agent.description || '', provider: agent.provider || null, providerModel: agent.providerModel || '', systemPrompt: agent.systemPrompt || '', knowledgeStoreIds: agent.knowledgeStoreIds || [], toolIds: agent.toolIds || [] })
  else Object.assign(agentForm, { name: '', description: '', provider: null, providerModel: '', systemPrompt: '', knowledgeStoreIds: [], toolIds: [] })
  showAgentDialog.value = true
}

async function saveAgent() {
  try {
    if (editingAgent.value?.id) { await agentsStore.updateAgent(editingAgent.value.id, { ...agentForm }); notify.success('Agent updated!') }
    else { await agentsStore.createAgent({ ...agentForm }); notify.success('Agent created!') }
    showAgentDialog.value = false
  } catch { notify.error('Failed to save agent.') }
}
...
function openToolDialog(tool = null) {
  ...
  if (tool) Object.assign(toolForm, { name: tool.name, description: tool.description || '', type: tool.type, config: { serverUrl: tool.config?.serverUrl || '', toolName: tool.config?.toolName || '', code: tool.config?.code || '' }, enabled: tool.enabled !== false })
  else Object.assign(toolForm, { name: '', description: '', type: 'mcp', config: { serverUrl: '', toolName: '', code: '' }, enabled: true })
...
async function saveTool() {
  ...
  try {
    if (editingTool.value?.id) { await toolsStore.updateTool(editingTool.value.id, payload); notify.success('Tool updated!') }
    else { await toolsStore.createTool(payload); notify.success('Tool created!') }
    showToolDialog.value = false
  } catch { notify.error('Failed to save tool.') }
}
...
const ksRightPanelHint = computed(() => knowledgeStore.knowledgeStores.length ? 'Select a store to manage its documents' : 'Create a knowledge store first')

async function selectKs(ks) { selectedKs.value = ks; await knowledgeStore.loadDocuments(ks.id) }
async function createKs() {
  const vectorStoreConfig = { type: newKsVectorStoreType.value }
  if (newKsVectorStoreType.value === 'chroma' && newKsVectorStoreUrl.value) vectorStoreConfig.url = newKsVectorStoreUrl.value
  const documentProcessorConfig = { type: newKsDocProcessorType.value }
  if ((newKsDocProcessorType.value === 'tika' || newKsDocProcessorType.value === 'docling') && newKsDocProcessorUrl.value) documentProcessorConfig.url = newKsDocProcessorUrl.value
  ...
  showCreateKs.value = false; newKsName.value = ''; newKsDesc.value = ''
  // Reset to defaults
  newKsEmbeddingModelId.value = defaultEmbeddingModelId.value
  newKsVectorStoreType.value = defaultVectorStoreType.value || 'local'
  newKsVectorStoreUrl.value = vectorStoreUrls[defaultVectorStoreType.value] || ''
...
async function handleDrop(e) { isDragging.value = false; if (!selectedKs.value) return; for (const file of e.dataTransfer.files) await knowledgeStore.uploadDocument(selectedKs.value.id, file) }
async function handleFileSelect(e) { if (!selectedKs.value) return; for (const file of e.target.files) await knowledgeStore.uploadDocument(selectedKs.value.id, file) }
function statusVariant(status) { return { ready: 'success', indexed: 'success', processing: 'warning', failed: 'destructive', error: 'destructive' }[status] || 'outline' }

onMounted(async () => {
  await Promise.all([
    settingsStore.loadSettings(), settingsStore.loadWebhooks(),
    modelsStore.loadModels(), agentsStore.loadAgents(),
    toolsStore.loadTools(), knowledgeStore.loadKnowledgeStores(),
  ])
})
</script>
