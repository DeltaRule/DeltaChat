  ...
  <div class="h-screen flex flex-col overflow-hidden bg-background text-foreground">
    ...
    <TooltipProvider :delay-duration="0">
      ...
      <header class="sticky top-0 z-50 flex h-12 items-center border-b border-border bg-background/80 backdrop-blur-lg px-4 gap-3 shrink-0">
        ...
        <Button v-if="authStore.isAdmin" :variant="isAdmin ? 'secondary' : 'ghost'" size="icon-sm" @click="router.push('/admin')">
        ...
        <DropdownMenu v-if="authStore.isAuthenticated">
          ...
          <DropdownMenuContent align="end" class="w-48">
            <div class="px-2 py-1.5 text-sm">
              <div class="font-medium truncate">{{ authStore.user?.name || authStore.user?.email }}</div>
              <div class="text-xs text-muted-foreground capitalize">{{ authStore.user?.role }}</div>
            </div>
